...
const VendorProductChangedForm = ({ match }) => {
  ...
  const submitHandler = async (e) => {
    ...
    if (foto_produk.length === 0) {
      ...
      for (let i = 0; i < productData.foto_produk.length; i++) {
        formData.append(`files`, foto_produk[i], foto_produk[i].name);
      }
      const fileRes = await axios
        .post("https://bikinevent.id/api/upload", formData, {
          headers: {
        ...
        .then(async (res) => {
          ...
          const responseEdit = await productService.editProductById(
            match.params.id,
            loginInfo.token,
            body
          );
        ...
        <MainVendash>
          <AnimationPortofolio>
            ...
            <form>
              ...
              <Buttonslog type="button" onClick={submitHandler}>
